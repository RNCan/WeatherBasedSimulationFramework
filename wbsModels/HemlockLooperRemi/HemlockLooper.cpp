...
namespace WBSF
{
	...
	const double CHemlockLooperModel::B[NB_PARAMS] = { 0.637, 2.591, 1.564, 1.414, 1.531, 1.2440 };	//variability
					...
					for (size_t i = 0; i<m_SAResult.size(); i++)
					{
						for (size_t p = P_EGG_L1; p <= m_lastParam; p++)
						{
								...
								statSim.IsInside(m_SAResult[i].m_ref))
							{
								...
								double simV = statSim[m_SAResult[i].m_ref][CHemlockLooperCR::O_FIRST_STAGE + p + 1];
