//*****************************************************************************
// Class: CLeucopis, CLNFHost, CLNFStand
//
//*****************************************************************************
...
namespace WBSF
{
	...
	class CLNFHost;
	class CLNFStand;
	class CLeucopis : public CIndividual
	{
	public:


		CLeucopis(WBSF::CHost* pHost, CTRef creationDate = CTRef(), double age = LOF::EGG, TSex sex = RANDOM_SEX, bool bFertil = true, size_t generation = 0, double scaleFactor = 1);
		CLeucopis(const CLeucopis& in) :WBSF::CIndividual(in){ operator=(in); }
		CLeucopis& operator=(const CLeucopis& in);
		~CLeucopis(void);
		...
		virtual WBSF::CIndividualPtr CreateCopy()const override { return std::make_shared<CLeucopis>(*this); }

		inline CLNFHost* GetHost();
		inline const CLNFHost* GetHost()const;
		inline CLNFStand* GetStand();
		inline const CLNFStand* GetStand()const;
		inline const CLeucopisEquations& Equations()const;
	...
	protected:
		...
		//CTRef m_adultDate;
		CTRef m_adult_emergence;
		CTRef m_reachDate[LOF::NB_STAGES+1];
...
//			m_DD4(CDegreeDays::MODIFIED_ALLEN_WAVE, 4.0)
		...
		CLeucopisEquations m_equations;
		CDegreeDays m_DD;
		//CDegreeDays m_DD4;
	...
	inline CLNFHost* CLeucopis::GetHost(){ return static_cast<CLNFHost*>(m_pHost); }
	inline const CLNFHost* CLeucopis::GetHost()const{ return static_cast<const CLNFHost*>(m_pHost); }
	inline CLNFStand* CLeucopis::GetStand(){ ASSERT(m_pHost); return static_cast<CLNFStand*>(GetHost()->GetStand()); }
	inline const CLNFStand* CLeucopis::GetStand()const{ ASSERT(m_pHost); return static_cast<const CLNFStand*>(GetHost()->GetStand()); }
	inline const CLeucopisEquations& CLeucopis::Equations()const{ return GetStand()->m_equations; }
