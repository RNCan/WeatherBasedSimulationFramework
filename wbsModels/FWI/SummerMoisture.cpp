...
namespace WBSF
{
	...
	//south Hemishere
	//ERMsg ExecuteSH(CSCCallBack& callback)
	//{
	//
	//	//for all years
	//	double mdc_old = 15;
	//	double P = 0;
	//	for (int y = 0; y < dataPrcp.GetRows() - 1 && msg; y++)//-1 don't take the last year
	//	{
	//		if (!m_bOverwinter)
	//			mdc_old = 15;
	//
	//		int year = gridPrcp.GetFirstYear() + y;
	//
	//
	//		//CString line;
	//		//line.Format("%d,%.2lf,%.2lf,%d", nbPoint,pt.m_x, pt.m_y, year);
	//		//for all months
	//		for (int mm = 0; mm < dataPrcp.GetCols() - 1 && msg; mm++)
	//		{
	//			int m = (mm + 6) % 12;
	//			int yy = mm < 6 ? 0 : 1;
	//
	...
	//						double Qf = 800 * exp(-mdc_old / 400);
	//						double Qs = m_a * Qf + m_b * 3.94*P;
	//						mdc_old = max(0, 400 * log(800 / Qs));
	//					}
	//				}
	//
	//
	//				I = ComputeIndice(year + yy, m, mdc_old, dataPrcp[y + yy][m + 1], dataTmax[y + yy][m + 1], true);
	//				result[y + yy][0] = (float)(year + yy);
	//				result[y + yy][m + 1] = (float)I;
	//			}
	//			else
	//			{
	//				if (mm == 10)
	//					P = 0;
	//
	//				P += dataPrcp[y + yy][m + 1];//+1 the fisrt col is something else
	//
	//				result[y + yy][m + 1] = 0;
	//			}
	//
