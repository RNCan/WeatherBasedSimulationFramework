...
double CFWI::GetFFMC(double oldFFMC, const CHourlyData& day)
{
	return GetFFMC(oldFFMC, day[H_TAIR], day[H_RELH], day[H_WNDS], day[H_PRCP]);
}
...
double CFWI::GetDMC(double oldDMC, const CHourlyData& day)
{
	return GetDMC(oldDMC, day.GetTRef().GetMonth(), day[H_TAIR][MEAN], day[H_RELH][MEAN], day[H_PRCP][SUM]);
}
...
double CFWI::GetDC(double oldDC, const CHourlyData& day)
{
	return GetDC(oldDC, day.GetTRef().GetMonth(), day[H_TAIR][MEAN], day[H_PRCP][SUM]);
}
...
double CFWI::GetISI(double FFMC, const CHourlyData& day)
{
	return GetISI(FFMC, day[H_WNDS][MEAN]);
}
...
CTRef CFWI::GetLastDay(const CWeatherYear& weather)
{
	...
	CGrowingSeason GS(false, 0, 0, 0, m_TtypeEnd, m_nbDaysEnd, m_thresholdEnd);
...
short CFWI::GetInitialValue(const CWeatherStation& weather, size_t y, size_t lastDay, double& FFMC, double& DMC, double& DC)
{
	...
	else
	{
		...
		if( firstDay>=0)
		{
			short nbDay = GetNbDayLastRain(weather[y],  firstDay);
			FFMC = 85;
			DMC = 2*nbDay;
			DC = 5*nbDay;
		}
		else
		{
			...
			//a very cold place, probably no fire problem
			firstDay = 183;
			FFMC = 85;
			DMC = 6;
			DC = 15;
...
void CFWI::Execute(const CWeatherStation& weather, CModelStatVector& output)
{
	...
	output.clear();
	output.Init(weather.GetEntireTPeriod(), CFWIStat::NB_D_STAT, MISSING);
	...
	for(size_t y=0; y<weather.size(); y++)//for all years
	{
		...
		//for(size_t d=0; d<weather[y].GetNbDays(); d++)//for all day
		for (CTRef TRef = p.Begin(); TRef<=p.End(); TRef++)//for all day
		{
			if (m_method == ALL_HOURS_CALCULATION || 
				(m_method == NOON_CALCULATION) && TRef.GetHour()==12)
			{
			const CHourlyData& day = weather.GetHour(TRef);
			CTRef Tref = day.GetTRef();

			if (d >= firstDay && d <= lastDay)
			{

				//double Tnoon = day.GetTnoon();//GetTnoon(16, day.GetTMin(), day.GetTMax());
				//double Hrnoon = day.GetRHnoon(Tnoon);//GetHrnoon(day.GetTMin(), day.GetTMax(), Tnoon, day[RELH]);
		...
		bContinueMode= /*bAuto && */lastDay==weather[y].size()-1;
...
void CFWIStat::CovertD2M(const CModelStatVector& resultD, CModelStatVector& resultM)
{
...
void CFWIStat::CovertD2A(const CModelStatVector& resultD, CModelStatVector& resultA)
{
	_ASSERTE( resultD.GetNbStat() == CFWIStat::NB_D_STAT);
