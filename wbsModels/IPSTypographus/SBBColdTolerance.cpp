...
/*
void CSBBColdTolerance::ComputeAnnual(const CWeatherStation& weather)
{
	...
	for(int y=0; y<weather.GetNbYear(); y++) //There can be more than 1 year of weather input (actually there IS).
	{
		...
		{
			const CWeatherStationDay& T = weather[y].GetDay(d); //This is the current day's min and max
...
void CSBBColdTolerance::ExecuteDaily(const CWeatherStation& weather, CSBBSuperCoolingPointStat& stat)
{
	...
	for(size_t y=0; y<weather.size(); y++) //There can be more than 1 year of weather input (actually there IS).
	{
		...
		//size_t nbDay = weather[y].GetTRef().GetNbDays();
		CTPeriod p = weather[y].GetEntireTPeriod();
		if (y == 0)
			p.Begin() = CJDayRef(T_0, year);
		for (CTRef d = p.Begin(); d<=p.End(); d++) //Loop over days in weather year
		{
			const CWeatherDay& T = weather.GetDay(d); //This is the current day's min and max
			
			//Ct can drop below Lambda after Dec 31
			if(d.GetJDay()>=T_1) reverse=true; //Ct can now drop because we are after Dec 31st. T_1 in Equation [7]

			//New year reset when more than one full year is being simulated (multiple winters)
			if( y >= 1 && d.GetJDay() == T_0) 
			{
				Psurv=1;
				Ct=0;
			...
			//Microclimate
			double Tmax = T[H_TAIR][HIGHEST];
			double Tmin = T[H_TAIR][LOWEST];

			//If phloem temperature is being generated from air temperature
			if(m_bMicroClimate)
			{

				double Trange = T[H_TAIR][RANGE];
			...
			double p1=CFL::Max(0.,min(1.,(.5-Ct)/(.5-m_Lambda0)));   //Equation [9]
			double p3=CFL::Max(0.,min(1.,(Ct-0.5)/(m_Lambda1-0.5))); //Equation [9]
			double p2 = 1. - (p1+p3);                           //Equation [9]
