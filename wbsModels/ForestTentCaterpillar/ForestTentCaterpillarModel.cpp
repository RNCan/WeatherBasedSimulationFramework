...
ERMsg CForestTentCaterpillarModel::OnExecuteAnnual()
{
	...
	CFTCStatVector stat(m_weather.GetNbYear()-1, CTRef(m_weather[1].GetYear()));
/*
	CTRef hatchDate[8] = 
{
	CTRef(1967, MAY, 20),
...
*/
	for(int y=0; y<m_weather.GetNbYear()-1; y++)
	{
		...
		{
		case ORIGINAL:
			begin = CTRef(m_weather[y].GetYear()+1,	FIRST_MONTH, FIRST_DAY);
			threshold = 2.2;
			sumDD = 222.2;
			break;//orginal
		case REMI_PARAMETER: 
			begin = CTRef(m_weather[y].GetYear(),	NOVEMBER, 5);
			threshold = 3.6;
			sumDD = 99.14;
			break;
