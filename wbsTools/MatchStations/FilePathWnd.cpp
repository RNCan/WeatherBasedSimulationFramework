...
namespace WBSF
{
	...
	IMPLEMENT_DYNAMIC(CFilePathDlg, CDialogEx)
		...
		{
			return CDialogEx::Create(CFilePathDlg::IDD, pParent);
		}
		...
		void CFilePathDlg::OnChange(UINT ID)
		{
			...
			switch (ID)
			{
			case IDC_NORMALS_FILEPATH:	pDoc->SetNormalsDatabase(str);  break;
			case IDC_OBSERVATION_FILEPATH: pDoc->SetObservationDatabase(str);  break;
			}
		}
