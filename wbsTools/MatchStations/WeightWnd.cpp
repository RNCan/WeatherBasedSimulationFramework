...
static const COLORREF LINE_COLOR[MAX_WEIGHT_STATION] = { RGB(100, 100, 255), RGB(255, 0, 0), RGB(60, 255, 0), RGB(240, 255, 0), RGB(255, 150, 0), RGB(180, 0, 255), RGB(0, 255, 255),  RGB(0, 0, 0), RGB(255, 100, 100), RGB(50, 50, 255)};
	...
	if (graphics.empty())
	{
		...
		if (!m_observationStations.empty())
		{
			...
			for (size_t s = 0; s < min(size_t(10), m_observationStations.size()); s++)
			{
				...
				serie.m_lineStyle = (s % 2) ? CChartPointsExSerie::ltDash: CChartPointsExSerie::ltSolid;
			...
			for (CGraphSerieVector::iterator it2 = it1->m_series.begin(); it2 != it1->m_series.end(); it2++)
			{
				...
				if (it2->m_type == CGraph::XY || it2->m_type == CGraph::HISTOGRAM)
				{
					...
					for (CTRef TRef = period.Begin(); TRef <= period.End() /*&& ii < numberPoints*/; TRef++)
					{
						...
						double y = bIsInit ? weight[var][s][TRef]*100 : CHART_MISSING_VALUE;
						//it2->m_statistic
...
CWeightChartWnd::CWeightChartWnd()
{
...
BEGIN_MESSAGE_MAP(CWeightChartWnd, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
...
int CWeightChartWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CWeightChartWnd::OnSize(UINT nType, int cx, int cy)
{
	CDockablePane::OnSize(nType, cx, cy);
	AdjustLayout();
}

//void CWeightChartWnd::AdjustHorzScroll(CListBox& wndListBox)
//{
//	CClientDC dc(this);
...
void CWeightChartWnd::UpdateFonts()
{
	m_chartCtrl.SetFont(&afxGlobalData.fontRegular);
}


void CWeightChartWnd::OnWindowPosChanged(WINDOWPOS* lpwndpos)
{
...
BOOL CWeightChartWnd::PreTranslateMessage(MSG* pMsg)
{
...
void CWeightChartWnd::CreateToolBar()
{
...
void CWeightChartWnd::OnDateChange(UINT ID)
{
...
void CWeightChartWnd::AdjustLayout()
{
	if (GetSafeHwnd() == NULL)
	{
		return;
...
void CWeightChartWnd::OnGraphOptions()
{
...
void CWeightChartWnd::OnToolbarCommand(UINT ID, NMHDR *pNMHDR, LRESULT *pResult)
{
	OnToolbarCommand(ID);
}


void CWeightChartWnd::OnUpdateToolbar(CCmdUI *pCmdUI)
{
...
void CWeightChartWnd::OnToolbarCommand(UINT ID)
{
	CMatchStationDoc* pDoc = GetDocument();
	bool bInit = pDoc->GetCurIndex() != UNKNOWN_POS;
...
void CWeightChartWnd::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
...
void CWeightChartWnd::OnCopyGraph()
{
}

void CWeightChartWnd::OnSaveGraph()
{
	CAppOption option;
