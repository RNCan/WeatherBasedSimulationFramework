...
BOOL CResultToolBar::LoadToolBarEx(UINT uiToolbarResID, CMFCToolBarInfo& params, BOOL bLocked)
{
	...
	//*****************************
	CMFCToolBarDateTimeCtrl periodBegin(ID_TABLE_PERIOD_BEGIN, 5, DTS_SHORTDATECENTURYFORMAT );
	ReplaceButton(ID_TABLE_PERIOD_BEGIN, periodBegin);
	//*****************************
	CMFCToolBarDateTimeCtrl periodEnd(ID_TABLE_PERIOD_END, 6, DTS_SHORTDATECENTURYFORMAT);
	...
	//*****************************
	CMFCToolBarComboBoxButton statButton(ID_TABLE_STAT, 8, CBS_DROPDOWNLIST );
	...
	//*****************************
	CMFCToolBarComboBoxButton TMButton(ID_TABLE_TM_TYPE, 9, CBS_DROPDOWNLIST , 100);
...
{
	m_bEnableMessage = false;
	CHourlyEditorDoc* pDoc = (CHourlyEditorDoc*)GetDocument();
	CTPeriod period = pDoc->GetPeriod();
	bool bPeriodEnabled = pDoc->GetPeriodEnabled();

	if (lHint == CHourlyEditorDoc::LANGUAGE_CHANGE)
	{
	...
	if (lHint == CHourlyEditorDoc::INIT || lHint == CHourlyEditorDoc::DATA_PROPERTIES_ENABLE_PERIOD_CHANGE)
	{
	...
	if (lHint == CHourlyEditorDoc::INIT || lHint == CHourlyEditorDoc::DATA_PROPERTIES_PERIOD_CHANGE)
	{
	...
	if (lHint == CHourlyEditorDoc::INIT || lHint == CHourlyEditorDoc::DATA_PROPERTIES_STAT_CHANGE || lHint == CHourlyEditorDoc::LANGUAGE_CHANGE)
	{
	...
	if (lHint == CHourlyEditorDoc::INIT || lHint == CHourlyEditorDoc::PROPERTIES_TM_CHANGE || lHint == CHourlyEditorDoc::LANGUAGE_CHANGE)
	{
