...
CHourlyEditorDoc* CPropertiesWnd::GetDocument()
{
...
CWeatherDatabasePtr CPropertiesWnd::GetDatabasePtr()
{
...
{
	CHourlyEditorDoc* pDoc = CPropertiesWnd::GetDocument();
...
void CDataPropertyCtrl::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	...
	if (lHint == CHourlyEditorDoc::INIT || lHint == CHourlyEditorDoc::STATION_INDEX_CHANGE)
	{
...
void CPropertiesWnd::OnVialKillFocus()
{
	...
	if (pDB)
	{
		...
		if (!vialNoStr.IsEmpty())
		{
			...
			int curRow = VialNo - WBSF::ToInt(study.m_firstVial);
			if (curRow >= 0 && curRow<study.GetNbVials())
			{
				if (curRow != properties.m_curRow)
				{
					properties.m_curRow = curRow;
					GetDocument()->UpdateAllViews(NULL, CHourlyEditorDoc::PROPERTIES_CHANGE, NULL);
				}
			}
		}
	}
...
//CHourlyEditorDoc* CPropertiesWnd::GetDocument()
//{
//	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CHourlyEditorDoc)));
...
BOOL CPropertiesWnd::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
	...
	if (pFocus)
	{
		...
		if (pFocus == &m_wndPropList || pParent == &m_wndPropList)
		{
			if (m_wndPropList.OnCmdMsg(nID, nCode, pExtra, pHandlerInfo))
				return TRUE;
		}
	}
