...
//*********************************************************************
const char* CGCM4_ESM2_10km_MMGCreator::ATTRIBUTE_NAME[] = { "INPUT_PATH", "OUTPUT_FILEPATH"};
const char* CGCM4_ESM2_10km_MMGCreator::CLASS_NAME = "GCM4_ESM2_10KM_MMG_CREATOR";


CGCM4_ESM2_10km_MMGCreator::CGCM4_ESM2_10km_MMGCreator(void)
{
...
void CGCM4_ESM2_10km_MMGCreator::InitClass(const StringVector& option)
{
	GetParamClassInfo().m_className = GetString( IDS_SOURCENAME_GCM10_EXTRACTOR);
	...
	StringVector properties(IDS_PROPERTIES_GCM10_EXTRACTOR, ";|");
	...
	//GetParameters().push_back(CParamDef(CParamDef::COMBO, CParamDef::BY_STRING, ATTRIBUTE_NAME[1], properties[1], RPCName, "0"));
...
//CGCM4_ESM2_10km_MMGCreator::CGCM4_ESM2_10km_MMGCreator(const CGCM4_ESM2_10km_MMGCreator& in)
//{
//	operator=(in);
//}

void CGCM4_ESM2_10km_MMGCreator::Reset()
{
	m_path.clear();
	//m_inputRCP = "RCP 8.5";
...
CGCM4_ESM2_10km_MMGCreator& CGCM4_ESM2_10km_MMGCreator::operator =(const CGCM4_ESM2_10km_MMGCreator& in)
{
	if( &in != this)
	{
...
bool CGCM4_ESM2_10km_MMGCreator::operator ==(const CGCM4_ESM2_10km_MMGCreator& in)const
{
	bool bEqual = true;
...
string CGCM4_ESM2_10km_MMGCreator::GetValue(size_t type)const
{
	string str;
...
void CGCM4_ESM2_10km_MMGCreator::SetValue(size_t type, const string& str)
{
	ASSERT( NB_ATTRIBUTE == 2); 
	switch(type)
	{
...
bool CGCM4_ESM2_10km_MMGCreator::Compare(const CParameterBase& in)const
{
	ASSERT(in.GetClassID() == GetClassID() );
	const CGCM4_ESM2_10km_MMGCreator& info = dynamic_cast<const CGCM4_ESM2_10km_MMGCreator&>(in);
	return operator==(info);
}

CParameterBase& CGCM4_ESM2_10km_MMGCreator::Assign(const CParameterBase& in)
{
	ASSERT(in.GetClassID() == GetClassID() );
	const CGCM4_ESM2_10km_MMGCreator& info = dynamic_cast<const CGCM4_ESM2_10km_MMGCreator&>(in);
	return operator=(info);
...
ERMsg CGCM4_ESM2_10km_MMGCreator::Execute(CFL::CCallback& callback)
{
	ERMsg msg;
...
const char* CGCM4_ESM2_10km_MMGCreator::VARIABLES_NAMES[NB_VARIABLES] = { "tmin", "tmax", "prec", "vapr", "swind" };//, "srad" 
const char* CGCM4_ESM2_10km_MMGCreator::RCP_NAME[NB_RCP] = { "rcp26", "rcp45", "rcp85" };



string CGCM4_ESM2_10km_MMGCreator::GetFileName(int period, int RCP, int var)
{
	string name;
	if (period == HISTORICAL)
		name = string("canesm2_hist_") + VARIABLES_NAMES[var] + "_abs_1951-2005.nc";
	else
		name = string("canesm2_") + RCP_NAME[RCP] + "_" + VARIABLES_NAMES[var] + "_abs_2006-2100.nc";

	return name;
}
string CGCM4_ESM2_10km_MMGCreator::GetProjectionWKT()
{
...
CGeoExtents CGCM4_ESM2_10km_MMGCreator::GetGCM10Extents()
{
	CGeoExtents extents;
...
void CGCM4_ESM2_10km_MMGCreator::GetOptions(CBaseOptions& options)
{
...
void CGCM4_ESM2_10km_MMGCreator::ConvertData(CMonthlyVariable& data)const
{
...
string CGCM4_ESM2_10km_MMGCreator::GetFilePath( string fileTitle)
{
...
ERMsg CGCM4_ESM2_10km_MMGCreator::CreateMMG(int rcp, CMonthlyMeanGrid& MMG, CFL::CCallback& callback)
{
	ERMsg msg;
...
void CGCM4_ESM2_10km_MMGCreator::GetDEM(CFL::CMatrix<float>& DEM)
{
