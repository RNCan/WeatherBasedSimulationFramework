...
namespace WBSF
{
	...
	//*********************************************************************
	const char* CUIACISDaily::ATTRIBUTE_NAME[NB_ATTRIBUTES] = { "UserName", "Password", "WorkingDir", "FirstYear", "LastYear" };
	const size_t CUIACISDaily::ATTRIBUTE_TYPE[NB_ATTRIBUTES] = { T_STRING, T_PASSWORD, T_PATH, T_STRING, T_STRING };
	const UINT CUIACISDaily::ATTRIBUTE_TITLE_ID = IDS_UPDATER_ACIS_DAILY_P;
	const UINT CUIACISDaily::DESCRIPTION_TITLE_ID = ID_TASK_ACIS_DAILY;

	const char* CUIACISDaily::CLASS_NAME(){ static const char* THE_CLASS_NAME = "ACISDaily";  return THE_CLASS_NAME; }
	CTaskBase::TType CUIACISDaily::ClassType()const { return CTaskBase::UPDATER; }
	static size_t CLASS_ID = CTaskFactory::RegisterTask(CUIACISDaily::CLASS_NAME(), (createF)CUIACISDaily::create);
	
	const char* CUIACISDaily::SERVER_NAME = "www.agric.gov.ab.ca";

	//www.
	CUIACISDaily::CUIACISDaily(void)
	{}

	CUIACISDaily::~CUIACISDaily(void)
	{}

	long CUIACISDaily::GetNbDay(const CTime& t)
	{
		return GetNbDay(t.GetYear(), t.GetMonth() - 1, t.GetDay() - 1);
	}

	long CUIACISDaily::GetNbDay(int y, int m, int d)
	{
		ASSERT(m >= 0 && m < 12);
		ASSERT(d >= 0 && d < 31);
	...
	//std::string CUIACISDaily::Option(size_t i)const
	//{
	//	string str;
	//	switch (i)
	//	{
	//	case TYPE:		str = "Hourly|Daily"; break; // GetString(IDS_CONVERT_DIRECTION); break;
	//	};
	//	return str;
	//}

	std::string CUIACISDaily::Default(size_t i)const
	{
		string str;
	...
	ERMsg CUIACISDaily::GetStationList(CLocationVector& stationList, CCallback& callback)const
	{
		ERMsg msg;
		...
		callback.PushTask(GetString(IDS_LOAD_STATION_LIST), 1);
	...
	string CUIACISDaily::GetSessiosnID(CHttpConnectionPtr& pConnection)
	{
		string sessionID;
		string page;
	...
	ERMsg CUIACISDaily::Execute(CCallback& callback)
	{
		ERMsg msg;
	...
	//ERMsg CUIACISDaily::DownloadStationHourly(CCallback& callback)
	//{
	//	ERMsg msg;
	...
	ERMsg CUIACISDaily::DownloadStationDaily(/*CHttpConnectionPtr& pConnection, string sessionID, const CLocation& location,*/ CCallback& callback)
	{
		ERMsg msg;
		...
		//	bNeedDownload[y] = !FileExists(filePath);
	...
	bool CUIACISDaily::IsValid(const CLocation& info, short y, short m, const string& filePath)const
	{
	...
	string CUIACISDaily::GetOutputFilePath(int year, const string& ID)const
	{
		return GetDir(WORKING_DIR) + ToString(year) + "\\" + ID + ".csv";
	}

	string CUIACISDaily::GetOutputFilePath(int year, size_t m, const string& ID)const
	{
		return GetDir(WORKING_DIR) + ToString(year) + "\\" + FormatA("%02d", m + 1) + "\\" + ID + ".csv";
	}

	/*
	string CUIACISDaily::GetForecastFilePath( const string& stationID)const
	{
	string tmp;
	...
	*/
	bool CUIACISDaily::IsFileInclude(const string& filePath)const
	{
		bool bRep = false;
	...
	ERMsg CUIACISDaily::CleanList(StringVector& fileList, CCallback& callback)const
	{
		ERMsg msg;
	...
	ERMsg CUIACISDaily::GetStationList(StringVector& stationList, CCallback& callback)
	{
		ERMsg msg;
	...
	void CUIACISDaily::GetStationInformation(const string& stationID, CLocation& station)const
	{
	...
	ERMsg CUIACISDaily::GetWeatherStation(const std::string& ID, CTM TM, CWeatherStation& station, CCallback& callback)
	{
		ERMsg msg;
	...
	ERMsg CUIACISDaily::ReadData(const string& filePath, CYear& dailyData, CCallback& callback)const
	{
		ERMsg msg;
