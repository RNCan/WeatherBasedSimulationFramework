...
namespace WBSF
{
	...
	//**************************************************************
	class CCreateWeatherGridDB : public CTaskBase
	{
	public:

		enum TNbPoints { E_AT_LOCATION=0};
		//enum TOutput{ OT_HOURLY, OT_DAILY, NB_OUTPUT_TYPES };
		enum TATTRIBUTE { INPUT_FILEPATH, LOCATIONS_FILEPATH, OUTPUT_FILEPATH, VARIABLES, NB_POINTS, FIRST_YEAR, LAST_YEAR, INCREMENTAL, NB_ATTRIBUTES };
		static const char* CLASS_NAME();
		static CTaskPtr create(){ return CTaskPtr(new CCreateWeatherGridDB); }


		CCreateWeatherGridDB(void);
		virtual ~CCreateWeatherGridDB(void);
