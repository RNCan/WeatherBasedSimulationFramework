...
//*********************************************************************
const char* CMIROC_10km_MMGCreator::ATTRIBUTE_NAME[] = { "INPUT_PATH", "OUTPUT_FILEPATH"};
const char* CMIROC_10km_MMGCreator::CLASS_NAME = "MIROC_10KM_MMG_CREATOR";


CMIROC_10km_MMGCreator::CMIROC_10km_MMGCreator(void)
{
...
void CMIROC_10km_MMGCreator::InitClass(const StringVector& option)
{
	GetParamClassInfo().m_className = GetString( IDS_SOURCENAME_MIROC_10_EXTRACTOR);
	...
	StringVector properties(IDS_PROPERTIES_MIROC_10_EXTRACTOR, ";|");
...
//CMIROC_10km_MMGCreator::CMIROC_10km_MMGCreator(const CMIROC_10km_MMGCreator& in)
//{
//	operator=(in);
//}

void CMIROC_10km_MMGCreator::Reset()
{
	m_path.clear();
	//m_inputRCP = "RCP 8.5";
...
CMIROC_10km_MMGCreator& CMIROC_10km_MMGCreator::operator =(const CMIROC_10km_MMGCreator& in)
{
	if( &in != this)
	{
...
bool CMIROC_10km_MMGCreator::operator ==(const CMIROC_10km_MMGCreator& in)const
{
	bool bEqual = true;
...
string CMIROC_10km_MMGCreator::GetValue(size_t type)const
{
	string str;
...
void CMIROC_10km_MMGCreator::SetValue(size_t type, const string& str)
{
	ASSERT( NB_ATTRIBUTE == 2); 
	switch(type)
	{
...
bool CMIROC_10km_MMGCreator::Compare(const CParameterBase& in)const
{
	ASSERT(in.GetClassID() == GetClassID() );
	const CMIROC_10km_MMGCreator& info = dynamic_cast<const CMIROC_10km_MMGCreator&>(in);
	return operator==(info);
}

CParameterBase& CMIROC_10km_MMGCreator::Assign(const CParameterBase& in)
{
	ASSERT(in.GetClassID() == GetClassID() );
	const CMIROC_10km_MMGCreator& info = dynamic_cast<const CMIROC_10km_MMGCreator&>(in);
	return operator=(info);
...
ERMsg CMIROC_10km_MMGCreator::Execute(CFL::CCallback& callback)
{
	ERMsg msg;
...
const char* CMIROC_10km_MMGCreator::VARIABLES_NAMES[NB_VARIABLES] = { "tmin", "tmax", "prec", "vapr", "swind" };//, "srad" 
const char* CMIROC_10km_MMGCreator::RCP_NAME[NB_RCP] = { "rcp26", "rcp45", "rcp85" };



string CMIROC_10km_MMGCreator::GetFileName(int period, int RCP, int var)
{
	string name;
	if (period == HISTORICAL)
		name = string("miroc_esm_hist_") + VARIABLES_NAMES[var] + "_abs_1951-2005.nc";
	else
		name = string("miroc_esm_") + RCP_NAME[RCP] + "_" + VARIABLES_NAMES[var] + "_abs_2006-2100.nc";

	return name;
}
string CMIROC_10km_MMGCreator::GetProjectionWKT()
{
...
CGeoExtents CMIROC_10km_MMGCreator::GetGCM10Extents()
{
	CGeoExtents extents;
...
void CMIROC_10km_MMGCreator::GetOptions(CBaseOptions& options)
{
...
void CMIROC_10km_MMGCreator::ConvertData(CMonthlyVariable& data)const
{
...
string CMIROC_10km_MMGCreator::GetFilePath( string fileTitle)
{
...
ERMsg CMIROC_10km_MMGCreator::CreateMMG(int rcp, CMonthlyMeanGrid& MMG, CFL::CCallback& callback)
{
	ERMsg msg;
...
void CMIROC_10km_MMGCreator::GetDEM(CFL::CMatrix<float>& DEM)
{
