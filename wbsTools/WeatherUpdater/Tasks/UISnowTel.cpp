...
const size_t CUISnoTel::ATTRIBUTE_TYPE[NB_ATTRIBUTES] = { T_PATH, T_COMBO_INDEX, T_STRING_BROWSE, T_STRING, T_STRING, T_DATE };
...
std::string CUISnoTel::Option(size_t i)const
{
	...
	switch (i)
	{
	case TEMPORAL_TYPE:	str = "Hourly|Daily|Monthly"; break;// GetString(IDS_PROPERTIES_ENV_CAN_PRCP_RADAR); break;
...
std::string CUISnoTel::Default(size_t i)const
{
	...
	switch (i)
	{
	case TEMPORAL_TYPE: str = ToString(T_HOURLY); break;
	case FIRST_YEAR:
	...
	if (msg)
	{
		...
		while (!file.end())
		{
			...
			if (TRef.IsInit() && validPeriod.IsInside(TRef))
			{
				...
				if (IsMissing(Hmin) && IsMissing(Hmax) && IsMissing(Havg) &&
					!IsMissing(Hobs) && file.IsHourly() && Hobs > 0 && Hobs <= 100)//hummm????
				{
				...
				if (!IsMissing(Ea))
				{
					...
					accumulator.Add(TRef, H_EA, Ea * 1000);//Pa --> kPa
				}
				...
				{
					ASSERT(Es >= 0 && Es < 300);
					accumulator.Add(TRef, H_ES, Es * 1000);//Pa --> kPa
				}
			}
...
CTRef CSnoTelFile::GetTRef()const
{
	...
	size_t h = m_bHourlyData ? ToSizeT(at(V_TIME).substr(0, 2)) : 0;
	size_t mm = m_bHourlyData ? ToSizeT(at(V_TIME).substr(3, 2)) : 0;
	ASSERT(year >= 1900 && year <= 2099);
	ASSERT(m >= 0 && m<12);
	ASSERT(d >= 0 && d<GetNbDayPerMonth(year, m));
	ASSERT(mm >= 0 && mm <= 59);
	...
	CTRef TRef = m_bHourlyData ? CTRef(year, m, d, h) : CTRef(year, m, d);
