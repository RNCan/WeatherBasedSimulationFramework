...
namespace WBSF
{
	...
	ERMsg CHRCanUS::Execute(CCallback& callback)
	{
		...
		//date_to_update.insert(CTRef(2021, JANUARY, DAY_29).GetFormatedString("%Y%m%d"));
	...
	ERMsg CHRCanUS::CreateHourlyCanUS(set<string> date_to_update, CCallback& callback)
	{
		...
		for (auto it = hours_to_update.begin(); it != hours_to_update.end() && msg; it++)
		{
			...
			string command = "\"" + GetApplicationPath() + "gdalwarp.exe\" " + option + " " + argument + file_path_in_can + "\" \"" + file_path_tmp1 + "\"";
			msg += WinExecWait(command);
			msg += callback.StepIt(0);

			command = "\"" + GetApplicationPath() + "gdalwarp.exe\" " + option + " " + argument + file_path_in_usa + "\" \"" + file_path_tmp2 + "\"";
			msg += WinExecWait(command);
			msg += callback.StepIt(0);
			...
			if (msg)
			{
				...
				if (msg && var_in.count() > 6)
				{
					...
					if (msg)
					{
						...
						string command = "\"" + GetApplicationPath() + "gdal_translate.exe\" " + option + " " + argument + " \"" + file_path_out + "2" + "\" \"" + file_path_out + "\"";
						msg += WinExecWait(command);
				...
				Sleep(1000);//let the time to the system to finish task
				if (FileExists(file_path_tmp1))
					RemoveFile(file_path_tmp1);

				if (FileExists(file_path_tmp2))
					RemoveFile(file_path_tmp2);
