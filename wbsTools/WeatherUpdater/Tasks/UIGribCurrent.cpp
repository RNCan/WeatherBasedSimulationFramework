...
namespace WBSF
{
	...
	ERMsg CUIGribCurrent::CreateHourlyPrcp(const string& inputFilePath1, const string& inputFilePath2, const string& outputFilePath, CCallback& callback)const
	{
		...
		string command = "\"" + GetApplicationPath() + "ImageCalculator.exe\" " + option + " " + argument + " \"" + inputFilePath1 + "\" \"" + inputFilePath2 + "\" \"" + outputFilePath + "\"";
		msg += WinExecWait(command);
		msg += callback.StepIt(0);
	...
	void CUIGribCurrent::GetHRDPS(CHRDPS& HRDPS)const
	{
		...
		if (dimension == D_SURFACE)
		{
			CHRDPSVariables sfc("HGT_SFC|PRES_SFC|SNOD_SFC|TCDC_SFC");
			CHRDPSVariables tlg("DPT_AGL|RH_AGL|TMP_AGL|WDIR_AGL|WIND_AGL");

			HRDPS.m_variables = (sfc | tlg);
			HRDPS.m_heights.FromString("2|10");
			HRDPS.m_levels.clear();
		}
		else
		{
			CHRDPSVariables sfc("HGT_SFC|PRES_SFC");//APCP_SFC|DSWRF_SFC|SNOD_SFC|TCDC_SFC
