...
void CSplitterControl::DrawSplitterLine(CDC* pDC)
{
	...
	if (GetParent())
	{
		...
		//pDC->IntersectClipRect(rcParent);
	}
    ...
    for (int i = 0; i < 2; i++)
    {
        for (POSITION pos = this->m_listLinkedWnds[i].GetHeadPosition(); NULL != pos;)
        {
            ...
            if (SPS_VERTICAL&m_dwSplitterStyle)
            {
                ...
                {
					rcLinked.right = rcLast.left;
                }
                else
                {
					if (m_dwSplitterStyle&SPS_RIGHT_MOVE)
					{
						rcLinked.OffsetRect(rcLast.right - rcLinked.left, 0);
					}
					else
					{
						rcLinked.left = rcLast.right;
					}
                }
            }
            else
            {
                ...
                {
					if (m_dwSplitterStyle&SPS_DOWN_MOVE)
					{
						rcLinked.OffsetRect(0, rcLast.bottom - rcLinked.top);
					}
					else
					{
						rcLinked.top = rcLast.bottom;
					}
                }
            }
