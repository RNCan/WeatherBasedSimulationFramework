...
struct XHTMLTREEDATA
{
	...
	//XHTMLTREEDATA& operator=(const XHTMLTREEDATA& in)
	//{
	//	if( &in != this)
	//	{
	//		bChecked = in.bChecked;
	//		bEnabled = in.bEnabled;				
	//		bSeparator = in.bSeparator;				
	//		hTreeCtrl = in.hTreeCtrl;
	//		bExpanded = in.bExpanded;
	//		bHasBeenExpanded = in.bHasBeenExpanded;
	//		bModified = in.bModified;
	//		nChildren = in.nChildren;
	//		nChecked = in.nChecked;
	//		nSeparators = in.nSeparators;
	//		
	//		if (in.pszNote)
	//		{
	//			size_t len = _tcslen(in.pszNote);
	//			if (pszNote != NULL)
	//				delete [] pszNote;

	//			pszNote = new TCHAR [len + 4];
	//			memset(pszNote, 0, (len+4)*sizeof(TCHAR));
	//			_tcsncpy(pszNote, in.pszNote, len);
	//		}

	//		nTipWidth = in.nTipWidth;
	//		ds = in.ds;	
	//	}
	//}
...
public:
	...
	BOOL		GetAutoCheckChildren() { return m_bAutoCheckChildren; }
	...
	CXHtmlTree&	SetAutoCheckChildren(BOOL bFlag)
				{ m_bAutoCheckChildren = bFlag; return *this;}
...
protected:
	...
	BOOL			m_bAutoCheckChildren;	// TRUE = Auto check children when parent is checked
