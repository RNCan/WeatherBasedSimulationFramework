...
UINT UWM_XHTMLTREE_CHECKBOX		   = WM_USER+1;
...
//=============================================================================
{
	...
	HTREEITEM hItemCur = hItemSel ? (bDownDir ? GetNextItem(hItemSel) : GetPrevItem(hItemSel)) : NULL;
	CString sSearch = str;
...
//=============================================================================
{
	...
	if (pData)
	{
		...
		pXTCD->bExpanded = pData->bExpanded;
...
//=============================================================================
{
	...
	CollapseAll(); //is it really usefull??? RSA
	BOOL bOldDestroyingTree = m_bDestroyingTree;
	m_bDestroyingTree = TRUE;
...
//=============================================================================
{
	...
	else if (hItem && (TVHT_ONITEMSTATEICON & uFlags))
	{
		...
		// click on checkbox
		PostMessage(UWM_XHTMLTREE_CHECKBOX);
		return;
...
LRESULT CXHtmlTree::OnCheckClick(WPARAM wp, LPARAM lp)
{
	NMHDR NMHDR;
	LRESULT result;
	CXHtmlTree::OnClick(&NMHDR, &result);
	return TRUE;
}
	...
	if (pXTCD)		// will not be NULL if hItem is valid
	{
		...
		else if (flags & TVHT_ONITEMSTATEICON)
		{
			...
			if (pXTCD->bEnabled && !m_bReadOnly)
			{
				...
				if (bCtrlKey || m_bSmartCheck || !m_bAutoCheckChildren)
				{
					SetCheck(hItem, !pXTCD->bChecked);
				}
				else SetCheckChildren(hItem, !pXTCD->bChecked);//CHECK all children except if CTRL is hold
