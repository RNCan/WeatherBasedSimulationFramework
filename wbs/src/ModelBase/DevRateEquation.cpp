...
namespace WBSF
{
	const char* CDevRateEquation::EQUATION[NB_EQUATIONS][NB_INFO] =
	{
		...
		{ "Schoolfield_1981","Tk=T+273.16;(p25*Tk/298*exp(Ha/1.987*(1/298-1/Tk)))/(1+exp(HL/1.987*(1/TL-1/Tk))+exp(HH/1.987*(1/TH-1/Tk)))","p25=0.01[1E-4,1E4]|Ha=-5e4[-1e6,1e6]|HL=-7.5e4[-1e6,1e6]|TL=273[223,323]|HH=6e5[-1e6,1e6]|TH=308[273,373]", "list(over(rho[25]~bgroup('[',over(T[k],298),']')~e^{bgroup('(',over(H[A],1.987),')')~bgroup('(',over(1,298)~-~over(1,T[k]),')')},1+e^{bgroup('(',over(H[L],1.987),')')~bgroup('(',over(1,T[L])~-~over(1,T[k]),')')}+e^{bgroup('(',over(H[H],1.987),')')~bgroup('(',over(1,T[H])~-~over(1,T[k]),')')}))" },//, scriptstyle(T[k]==T+273.16)
		{ "Sharpe&DeMichele_1977","Tk=T+273.16;Tko=To+273.16;TkL=TL+273.16;TkH=TH+273.16;(p25*(Tk/Tko)*exp((Ha/1.987)*(1/Tko-1/Tk)))/(1+exp((HL/1.987)*(1/TkL-1/Tk))+exp((HH/1.987)*(1/TkH-1/Tk)))","p25=1E-4[1E-5,1E5]|To=0.2[0.001,50]|Ha=0.2[-1000,1E5]|HL=2[0.001,50]|TL=5[0.001,50]|HH=0.15[0.001,1E5]|TH=4[1,50]", "list(over(rho[25]~bgroup('[',over(T[k],T[k[o]]),']')~e^{bgroup('(',over(H[A],1.987),')')~bgroup('(',over(1,T[k[o]])~-~over(1,T[k]),')')},1+e^{bgroup('(',over(H[L],1.987),')')~bgroup('(',over(1,T[k[L]])~-~over(1,T[k]),')')}+e^{bgroup('(',over(H[H],1.987),')')~bgroup('(',over(1,T[k[H]])~-~over(1,T[k]),')')}))" },//, atop(atop(T[k]==T+273.16,T[k[o]]==T[o]+273.16),atop(T[k[b]]==T[b]+273.16,T[k[m]]==T[m]+273.16))
		...
		{ "Wagner_1988","Tk=T+273.16;p25*(Tk/298.15)*exp((Ha/1.987)*((1/298.15)-1/Tk))/((1+exp((HL/1.987)*((1/TL)-(1/Tk)))))","p25=0.1[0.0001,10]|Ha=28500[1,1E6]|HL=130000[1,2E6]|TL=305[250,350]", "list(over(rho[25]~bgroup('(',over(T[k],298.15),')')~e^{~bgroup('(',over(H[A],1.987),')')~bgroup('(',over(1,298.15)~-~over(1,T[k]),')')},1+e^{~bgroup('(',over(H[L],1.987),')')~bgroup('(',over(1,T[L])~-~over(1,T[k]),')')}))" },//, scriptstyle(T[k]==T+273.16)
