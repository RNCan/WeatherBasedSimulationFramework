...
namespace WBSF
{
	const char* CDevRateEquation::EQUATION[NB_EQUATIONS][NB_INFO] =
	{
		...
		{ "Schoolfield_1981","Tk=T+273.16;(p25*Tk/298*exp(Ha/1.987*(1/298-1/Tk)))/(1+exp(Hb/1.987*(1/Tb-1/Tk))+exp(Hm/1.987*(1/Tm-1/Tk)))","p25=0.01[1E-4,1E4]|Ha=-5e4[-1e6,0]|Hb=-7.5e4[-1E6,0]|Tb=273[223,323]|Hm=6e5[0,1E6]|Tm=308[273,373]", "list(over(rho[25]~bgroup('[',over(T[k],298),']')~e^{bgroup('(',over(H[A],1.987),')')~bgroup('(',over(1,298)~-~over(1,T[k]),')')},1+e^{bgroup('(',over(H[l],1.987),')')~bgroup('(',over(1,T[l])~-~over(1,T[k]),')')}+e^{bgroup('(',over(H[h],1.987),')')~bgroup('(',over(1,T[h])~-~over(1,T[k]),')')}))" },//, scriptstyle(T[k]==T+273.16)
