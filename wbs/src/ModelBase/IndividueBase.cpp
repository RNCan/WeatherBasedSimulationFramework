...
namespace WBSF
{
	...
	//bool CIndividue::m_bUseAttrition = false;
	...
	//*****************************************************************************
	CIndividue::CIndividue(CHost* pHost, CTRef creationDate, double age, size_t sex, bool bFertil, size_t generation, double scaleFactor)
	{
	...
	// Object destructor
	CIndividue::~CIndividue(void)
	{
	}


	CIndividue& CIndividue::operator=(const CIndividue& in)
	{
	...
	void CIndividue::Live(const CWeatherDay& weather)
	{
	...
	void CIndividue::Brood(const CWeatherDay& weather)
	{
		assert(IsCreated(weather.GetTRef()) && IsAlive());
		//m_broods = 0;	//reset brood for this day
	}

	void CIndividue::Die(const CWeatherDay& weather)
	{
		assert(IsCreated(weather.GetTRef()) && IsAlive());
	}
	//
	//void CIndividue::Live(const CDailyWaveVector& T, const CWeatherDay& weaDay)
	//{
	...
	bool CIndividue::CanPack(const CIndividuePtr& in)const
	{
	...
	void CIndividue::Pack(const CIndividuePtr& in)
	{
	...
	bool CIndividue::CanUnpack()const
	{
		return IsAlive();
	}

	CIndividuePtr CIndividue::Unpack()
	{
		m_scaleFactor /= 2;
		return CreateCopy();
	}
	...
	//*****************************************************************************
	/*bool CIndividue::IsDeadByAttrition(double RR)const
	{
	bool bDeath=false;
	...
	*/
	...
	CHost& CHost::operator = (const CHost& in)
	{
		if (&in != this)
		{
			...
			{
				CIndividuePtr pSpeciemen = (*it)->CreateCopy();
		...
		if (msg)
		{
			...
			while (getline(file, line) && !line.empty())
			{
				...
				for (int i = 0; i < nbBugs; i++)
					push_back(CIndividueInfo(TRef, age));
			}
