...
/*void ANNbd_shrink::ann_search(ANNdist box_dist)
{
  ...
  ANNpoint ANNkdQ;
	...
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_Psearch(inner_dist);	// search inner child first
		child[ANN_OUT]->ann_Psearch(box_dist);	// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_Psearch(box_dist);	// search outer child first
		child[ANN_IN]->ann_Psearch(inner_dist);	// ...then outer child
	}
