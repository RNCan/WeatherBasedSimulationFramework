...
namespace WBSF
{
	...
	double eᵒ(double Tmin, double Tmax);//daily vapor pressure
	...
	inline long GetDegrees(double dec, double mult = 1)
	{
		long nbSec = long(dec * 3600 * mult + Signe(dec)*0.5);
	...
	inline int GetMinutes(double dec, double mult = 1)
	{
		...
		return long((nbSec - abs(GetDegrees(dec, mult)) * 3600 * mult) / (60 * mult));
	}
	...
	double Gamma(double);
	inline double Factorial(double x) { return Gamma(x + 1); }
	inline double LogFactorial(double x) { return LogGamma(x + 1); }
