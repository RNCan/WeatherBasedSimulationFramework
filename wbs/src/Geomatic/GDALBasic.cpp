...
ERMsg CGeoCoordFile::Load(const string& filePath, const string X, const string Y)
{
	...
	if (msg)
	{
		...
		if (coordinatePos.size() != 2)
		{
			msg.ajoute("Invalid header for coordinates (X,Y). " + ToString(coordinatePos.size()) + " column(s) match for header \"" + X.c_str() + "\" and \"" + Y.c_str() + "\".");
			return msg;
		}
		...
		for (int i = 0; loop != CSVIterator(); ++loop, ++i)
		{
			if (loop->size() == loop.Header().size() )  //&& coordinatePos[0] < loop->size() && coordinatePos[1] < loop->size())
			{
				...
				xy.m_x = stod(loop->at(coordinatePos[0]));
				xy.m_y = stod(loop->at(coordinatePos[1]));
				m_xy.push_back(xy);
...
ERMsg CGeoCoordFile::Save(const string& filePath)
{
	ERMsg msg;
	...
	if (msg)
	{
		...
		//save projection
		if (m_prjIDSource != PRJ_NOT_INIT)
		{
			string prjFilePath(filePath);
			SetFileExtension(prjFilePath, ".prj");
			CProjectionPtr pPrj = GetProjection(m_prjIDSource);
