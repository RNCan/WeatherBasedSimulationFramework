...
namespace WBSF
{
		...
		if (msg)
		{
			...
			for (CSVIterator loop(file); loop != CSVIterator() && msg; ++loop)
			{
				if ((*loop).size() == 2)
				{
					...
					at(TRef) = GetAbsolutePath(path, (*loop)[1]);
				}
			}
		}
			...
			if (msg)
			{
				for (CSVIterator loop(file, ",", true); loop != CSVIterator(); ++loop)
				{
					if (loop->size() == 5)
					{
						...
						at(TRef).m_filePath = (*loop)[1];
						at(TRef).m_time = stoi((*loop)[2]);
						at(TRef).m_size = stoi((*loop)[3]);
						at(TRef).m_attribute = stoi((*loop)[4]);
					}
				}
		...
		if (m_extents.IsInside(xy))
		{
			...
			for (size_t v = 0; v < m_bands.size(); v++)
			{
				if (m_bands[v] != NOT_INIT)
				{
					...
					data[v] = m_lines[xy.m_y]->at(m_bands[v])->get_value(xy.m_x);
					if (v == H_PRCP)
						data[v] *= 3600; //convert mm/s into mm/h
	...
	float CSfcDatasetCached::get_variable(const CGeoPointIndex& xy, size_t v)const
	{
		...
		size_t b = get_band(v);
		if (!m_extents.IsInside(xy) || m_bands[b] == UNKNOWN_POS)
		...
		return m_lines[xy.m_y]->at(b)->get_value(xy.m_x);
			...
			for (size_t v = 0; v < m_bands.size(); v++)
			{
				if (m_bands[v] != NOT_INIT)
				{
					...
					if (m_bVRT)
					{
						...
						m_lines[y]->at(b).reset(pBlockTmp);
					}
					else
					{
