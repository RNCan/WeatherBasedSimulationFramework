...
namespace WBSF
{
	...
	namespace Landsat1
	{
		...
		typedef std::array< CBandStat, Landsat1::SCENES_SIZE> CBandStats;




		typedef __int16 LandsatDataType;
		typedef std::array<LandsatDataType, Landsat1::SCENES_SIZE> LandsatPixel;
		...
		public:
			...
			LandsatDataType operator[](const Landsat1::TIndices& i)const;
			LandsatDataType operator[](const Landsat1::TIndices& i) { return ((const CLandsatPixel*)(this))->operator[](i); }

			bool IsInit()const;
			bool IsInit(Landsat1::TIndices i)const;
			bool IsValid()const;
			bool IsBlack()const { return (at(Landsat1::B1) == 0 && at(Landsat1::B2) == 0 && at(Landsat1::B3) == 0); }
			bool IsZero()const { return (at(Landsat1::B1) == 0 && at(Landsat1::B2) == 0 && at(Landsat1::B3) == 0 && at(Landsat1::B4) == 0 && at(Landsat1::B5) == 0 && at(Landsat1::B6) == 0 && at(Landsat1::B7) == 0); }
			...
			void correction8to7(Landsat1::TCorr8 type);
		...
		public:
			...
			Landsat1::TIndices	m_type;
			std::string			m_op;
			double				m_threshold;
			double				m_trigger;

			CIndices(Landsat1::TIndices	type, std::string op, double threshold, double trigger)
			{
				ASSERT(op == ">" || op == "<");
		...
		public:
			...
			CLandsatFileInfo()
			{
				m_format = Landsat1::F_UNKNOWN;
				m_captor = -32768;
				m_path = -32768;
				m_row = -32768;
			}

			Landsat1::TLandsatFormat m_format;
			__int16 m_captor;
			__int16 m_path;
			__int16 m_row;
			CTRef m_TRef;
		...
		public:
			...
			//static const char* SCENE_NAME[Landsat1::SCENES_SIZE];
			...
			ERMsg CreateIndices(size_t i, const std::string filePath, Landsat1::TIndices type);
