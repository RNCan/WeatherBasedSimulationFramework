	...
	public:
		...
		double calc(const dlib::matrix<double>& params, double xx)const
		{
			...
			case CVariogram::CUBIC:
			{
				...
				if (xx <= range)
					yh = nugget + sill * (7.0 * pow(xx / range, 2.0) - 35.0 / 4.0 * pow(xx / range, 3.0) + 7.0 / 2.0 * pow(xx / range, 5.0) - 3.0 / 4.0 * pow(xx / range, 7.0));
				else
					yh = nugget + sill;
			...
			case CVariogram::PENTASPHERICAL:
			{
				...
				if (xx <= range)
					yh = nugget + sill * (15.0 / 8.0 * pow(xx / range, 1.0) - 5.0 / 4.0 * pow(xx / range, 3.0) + 3.0 / 8.0 * pow(xx / range, 5.0));
				else
					yh = nugget + sill;
		...
		void ComputeParams(dlib::matrix<double>& params, dlib::matrix<double>& lower_bound, dlib::matrix<double>& upper_bound)const
		{
			...
			case CVariogram::POWER:
			{
				...
				//params(2) = 0.25;
				params(1) = (sill - nugget) / pow(ma, 0.5);
				params(2) = 0.5;
				break;
			}
			case CVariogram::CUBIC:
...
#pragma omp parallel for 
	...
	*/
	...
	double  CVariogram::cova3(const CGridPoint& pt1, const CGridPoint& pt2)const
	{
		...
		case SPERICAL:			cova3 = (hr < 1.0) ? cc * (1.0 - (3.0 / 2.0 * pow(hr, 1.0) - 1.0 / 2.0 * pow(hr, 3.0))) : 0; break;
	...
	//		break;
	//	}
	//	case CUBIC:				cova3 = (h / aa < 1.0) ? cc * (1.0 - (7 * pow(h / aa, 2) - 35 / 4 * pow(h / aa, 3) + 7 / 2 * pow(h / aa, 5) - 3 / 4 * pow(h / aa, 7))) : 0; break;
	//	case PENTASPHERICAL:	cova3 = (h / aa < 1.0) ? cc * (1.0 - (15.0 / 8.0 * pow(h / aa, 1.0) - 5.0 / 4.0 * pow(h / aa, 3.0) + 3.0 / 8.0 * pow(h / aa, 5.0))) : 0; break;
	//	case SINE_HOLE_EFFECT:	cova3 = (h / aa < 1.0) ? cc * (1.0 - (1.0 - sin(PI*h / aa) / (PI*h / aa))) : 0; break;
