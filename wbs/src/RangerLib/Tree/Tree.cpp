    ...
    while (1) {
      ...
      if (prediction_data->isOrderedVariable(split_varID)) {
        if (value <= split_values[nodeID]) {
		  ...
		  size_t splitID = (size_t)floor(split_values[nodeID]);
...
bool Tree::splitNode(size_t nodeID) {
  ...
  if (data->isOrderedVariable(split_varID)) {
    ...
    for (auto& sampleID : sampleIDs[nodeID]) {
	  ...
	  size_t splitID = (size_t)floor(split_value);
...
// Reserve space, reserve a little more to be save)
  ...
  oob_sampleIDs.reserve((size_t)(num_samples * (exp(-sample_fraction) + 0.1)));
...
// Reserve space, reserve a little more to be save)
  ...
  //std::initializer_list<double> weightsRemi(&(*case_weights->begin()), &(*case_weights->end()));
  std::initializer_list<double> weightsRemi(case_weights->data(), case_weights->data() + case_weights->size());
