...
void ForestSurvival::init_internal_grow(Data* data) {

	//status_var_name = status_variable_name;
	// Convert status variable name to ID
	if (!prediction_mode && !status_var_name.empty()) {
		status_varID = data->getVariableID(status_var_name);
		if (status_varID >= data->getNumCols())
		{
			throw std::runtime_error("Status variable " + status_var_name + " not found.");
		}
		
		independent_variable_names.erase(independent_variable_names.begin()+ status_varID);
	}
	...
	if (mtry == 0) {
		unsigned long temp = (unsigned long)ceil(sqrt((double)(data->getNumCols() - 2)));
	...
	if (!prediction_mode) {
		for (size_t i = 0; i < data->getNumRows(); ++i) {
			...
			uint timepointID = (uint)(find(unique_timepoints.begin(), unique_timepoints.end(), value) - unique_timepoints.begin());
...
void ForestSurvival::growInternal(Data* data) {
	trees.reserve(num_trees);
	for (size_t i = 0; i < num_trees; ++i) {
...
void ForestSurvival::init_internal_predict(const Data* data) {
	size_t num_prediction_samples = data->getNumRows();
	size_t num_timepoints = unique_timepoints.size();
	if (!status_var_name.empty())
		status_varID = data->getVariableID(status_var_name);
...
void ForestSurvival::predictInternal(size_t i, const Data* data) {
	...
	if (predict_all) {
		...
		for (size_t k = 0; k < num_trees; ++k) {
			predictions[0][i][k] = (double)(((TreeSurvival*)trees[k])->getPredictionTerminalNodeID(i));
		}
	}
	else {
		for (size_t j = 0; j < num_timepoints; ++j) {
...
void ForestSurvival::computePredictionErrorInternal(Data* data) {
	...
	predictions = std::vector<std::vector<std::vector<double>>>(1, std::vector<std::vector<double>>(data->getNumRows(), std::vector<double>(num_timepoints, 0)));
...
void ForestSurvival::writeOutputInternal() {
	if (verbose_out)
	{
		...
		*verbose_out << "Status variable name:              " << status_var_name << std::endl;
		*verbose_out << "Status variable ID:                " << status_varID << std::endl;
	}
}

void ForestSurvival::writeConfusionFile(std::string filename) {
