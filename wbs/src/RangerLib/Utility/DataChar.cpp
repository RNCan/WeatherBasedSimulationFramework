...
void DataChar::reshape(const std::vector<std::string>& names)
{
	assert(data != nullptr);

	//cols to remove
	char* p_new = new char[names.size() * num_rows];
	...
	for (auto name : names)
	{
		...
		if (it != variable_names.end())
		{
			...
			memcpy(&(p_new[i * num_rows]), &(data[col * num_rows]), num_rows * sizeof(char));
		}
		i++;
	}
