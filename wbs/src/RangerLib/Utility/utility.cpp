...
void drawWithoutReplacementFisherYates(std::vector<size_t>& result, std::mt19937_64& random_number_generator,
	...
	for (size_t i = 0; i < num_samples; ++i) {
		size_t j = i + (size_t)(distribution(random_number_generator) * (max - skip.size() - i));
...
void drawWithoutReplacementWeighted(std::vector<size_t>& result, std::mt19937_64& random_number_generator,
	...
	std::initializer_list<double> weightsRemi(weights.data(), weights.data() + weights.size());
	std::discrete_distribution<> weighted_dist(weightsRemi);
	//std::discrete_distribution<double> weighted_dist(weights.begin(), weights.end());
	...
	for (auto& variable_name : unordered_variable_names) {
		...
		if (all_values.size() > max_level_count) {
			return "Too many levels in unordered categorical variable " + variable_name + ". Only "
				+ uintToString((uint)max_level_count) + " levels allowed on this system.";
		}
...
double maxstatPValueLau94(double b, double minprop, double maxprop, size_t N, std::vector<size_t>& m) {
	...
	for (size_t i = 0; i < m.size() - 1; ++i) {
		double m1 = (double)m[i];
		double m2 = (double)m[i + 1];
...
void maxstat(std::vector<double>& scores, std::vector<double>& x, std::vector<size_t>& indices, double& best_maxstat,
	...
	size_t minsplit = 0;
	if (n * minprop > 1) {
		minsplit = (size_t)(n * minprop - 1);
	}
	size_t maxsplit = (size_t)(n * maxprop - 1);
