...
ERMsg CModelParameterization::GetFValue(const vector<double>& X, double& F, CStatisticXY& stat, CCallback& callback)
{
	...
	if( m_bUseHxGrid )
	{
		...
		//std::string dllName = "models\\" + m_model.GetDLLName();
		std::string dllName = m_model.GetDLLFilePath();
						
		int loop = 0;//to determine if hxGrid is operational
		for (size_t i = 0; i<gSession.m_stats.size()&&msg; i++)
		{
			DWORD hxGridSessionId=0;
			IGenericStream* inStream = (IGenericStream*)CreateStream(gSession.m_sessionId, i, X);
			//int test = inStream->AddRef();
			//test = inStream->Release();
			
			while( msg && pGridUser->RunTask(dllName.c_str(),"RunSimulatedAnnealing",inStream,FinalizeSimulatedAnnealing,&hxGridSessionId,false) != S_OK )
			{
		...
		int lastComplet= gSession.m_nbTaskCompleted;
		...
		while( msg && !bComplet )
		{
			...
			msg+=callback.StepIt(0);
			msg+=gSession.m_msg;
			if( lastComplet!= gSession.m_nbTaskCompleted)
			{
				lastComplet= gSession.m_nbTaskCompleted;
			}
