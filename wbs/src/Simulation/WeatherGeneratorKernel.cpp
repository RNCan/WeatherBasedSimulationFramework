...
namespace WBSF
{
	...
	void CWeatherGeneratorKernel::Generate(CWeatherYear& dailyData)
	{
		...
		//temperature part
		if (m_variables[H_TAIR] || m_variables[H_TRNG])
		{
			ASSERT(m_normals.GetVariables()[H_TAIR]);
			...
			for (size_t m = 0; m < 12; m++) //loop over 12 months
			{
				...
				for (size_t d = 0; d < GetNbDayPerMonth(m); d++)
				{
					...
					if (m_variables[H_TRNG])
						dailyData[m][d][H_TRNG] = tmax - tmin;
		...
		if (m_variables[H_TDEW] || m_variables[H_RELH])
		{
			...
			for (size_t m = 0; m < 12; m++)
			{
				...
				for (size_t d = 0; d < GetNbDayPerMonth(m); d++)
				{
					_ASSERTE(dailyData[m][d][H_TAIR][LOWEST] > MISSING);
					_ASSERTE(dailyData[m][d][H_TAIR][HIGHEST] > MISSING);
					...
					dailyData[m][d][H_TDEW] = Hr2Td(dailyData[m][d][H_TAIR][MEAN], Hr);
				}
			}
		}
	...
	void CWeatherGeneratorKernel::DailyPrecipitation(int m, const CNormalsData& normalTmp, CWeatherYear& dailyData)
	{
		...
		if (normalTmp[m][PRCP_TT] > 0.1)
		{
			...
			for (size_t d = 0; d < GetNbDayPerMonth(m); d++)
			{
				...
				double Pprecip = m_AP.GetPprecip(dailyData[m][d][H_TAIR][LOWEST], dailyData[m][d][H_TAIR][HIGHEST], normalTmp[m][PRCP_TT], Sp);
