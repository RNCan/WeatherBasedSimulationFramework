...
namespace WBSF
{
	...
	class CInputAnalysisDlg;
	class CInputAnalysis : public CExecutable
	{
	...
	public:
		...
		static CExecutablePtr PASCAL CreateObject(){ return CExecutablePtr(new CInputAnalysis()); }
		...
		CInputAnalysis();
		CInputAnalysis(const CInputAnalysis& in);
		virtual ~CInputAnalysis();
		virtual const char* GetClassName()const{ return XML_FLAG; }

		virtual CExecutablePtr CopyObject()const{ return CExecutablePtr(new CInputAnalysis(*this)); }
		virtual CExecutable& Assign(const CExecutable& in){ ASSERT(in.GetClassName() == XML_FLAG); return operator=(dynamic_cast<const CInputAnalysis&>(in)); }
		virtual bool CompareObject(const CExecutable& in)const{ ASSERT(in.GetClassName() == XML_FLAG); return *this == dynamic_cast<const CInputAnalysis&>(in); }
		virtual void writeStruc(zen::XmlElement& output)const;
		virtual bool readStruc(const zen::XmlElement& input);

		void Reset();
		CInputAnalysis& operator =(const CInputAnalysis& in);
		bool operator == (const CInputAnalysis& in)const;
		bool operator != (const CInputAnalysis& in)const{ return !operator==(in); }
