...
enum TFilePath { FOREST_FILE_PATH, LANDSAT_FILE_PATH, PHYSICAL_FILE_PATH, OUTPUT_FILE_PATH, NB_FILE_PATH };
...
enum TDebug { D_NB_PAIRS, D_NB_DISTURBANCES, D_FIRST_DISTURBANCE, D_F_DATE1, D_F_DATE2, D_LAST_DISTURBANCE, D_L_DATE1, D_L_DATE2, NB_DEBUGS };
...
public:
	...
	//CDecisionTreeBlock GetDataRecord(size_t z1, CDecisionTreeBaseEx& DT)
	//{
	//	ASSERT(z1 < size());
	//	ASSERT(IsInit());
...
public:
	...
	void ProcessBlock(int xBlock, int yBlock, const CBandsHolder& bandHolder1, const CBandsHolder& bandHolder2, ForestVector& DT, vector< vector< CDADVector >>& disturbances);
...
ERMsg CDisterbanceAnalyser::ReadRules(ForestVector& DT)
{
	ERMsg msg;
