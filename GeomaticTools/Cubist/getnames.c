...
/*   -----------  */
{
	...
	if ( ! memcmp(Buffer, "discrete", 8) )
	{
	    ...
	    AttValName[MaxAtt][(MaxAttVal[MaxAtt]=1)] = _strdup("N/A");
	}
	else
	...
	AttValName[MaxAtt][++MaxAttVal[MaxAtt]] = _strdup(p);
	...
	do
	{
	    ...
	    AttValName[MaxAtt][MaxAttVal[MaxAtt]] = _strdup(Buffer);
	}
	while ( Delimiter == ',' );
