...
/*   -------  */
{
	...
	if (SAMPLE)
	{
		...
		WantTrain = (CaseNo)(SampleFrom * SAMPLE + 0.5);
	...
	while ((DVec = GetDataRec(Df, Train)))
	{
		...
		if (SAMPLE)
		{
			...
			if (SelectTrain != Train ||
				(!Train && drand48() >= WantTest / (float)LeftTest--))
			{
				FreeLastCase(DVec);
				continue;
			}
	...
	if (AttMean == Nil)
	{
		...
		ForEach(Att, 1, MaxAtt)
		{
			...
			if (Continuous(Att))
			{
				...
				if (Count > 1)
				{
					...
					AttMean[Att] = (ContValue)(floor((Sum / Count) * AttPrec[Att] + 0.1) / AttPrec[Att]);
					AttSD[Att] = (ContValue)(sqrt((SumSq - Sum * Sum / Count) / (Count - 1)) + 1E-6);
				}
				else
				{
					AttPrec[Att] = 0;
		...
		Precision = (int)(log(AttPrec[ClassAtt] / 10.0) / log(10.0) + 0.5);
	}
				...
				if (!strcmp(Name, "?"))
				{
					...
					if (!strcmp(Name, "N/A"))
					{
						...
						else
						{
							...
							if (TStampVal(Att))
							{
								Cv = (ContValue)(TStampToMins(Name));
								...
								if (DateVal(Att))
								{
									...
									{
										Cv = (ContValue)(strtod(Name, &EndVal));
