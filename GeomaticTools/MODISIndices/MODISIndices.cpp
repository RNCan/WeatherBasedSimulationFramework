...
namespace WBSF
{
	...
	CMODISIndicesOption::CMODISIndicesOption()
	{
		...
		m_appDescription = "This software computes indices (NBR, HDMI, ...) from MODIS images (composed of " + to_string(SCENES_SIZE) + " bands).";
		...
		{
			{ "Input Image", "srcfile", "", "ScenesSize(9)*nbScenes", "B1: MODIS band 1|B2: MODIS band 2|B3: MODIS band 3|B4: MODIS band 4|B5: MODIS band 5|B6: MODIS band 6|B7: MODIS band 7|QA: Image quality|Date: date of image(Julian day 1970 or YYYYMMDD format)|... for all scenes", "" },
	...
	ERMsg CMODISIndicesOption::ParseOption(int argc, char* argv[])
	{
		...
		MODIS::INDICES_FACTOR(m_iFactor);

		return msg;
	}

	ERMsg CMODISIndicesOption::ProcessOption(int& i, int argc, char* argv[])
	{
		ERMsg msg;
		...
		else if (IsEqual(argv[i], "-Despike"))
		{
			...
			if (type != I_INVALID)
			{
				m_despike.push_back(CMODISIndice(type, "<", threshold, min_trigger));
			}
			else
			{
		...
		if (!m_options.m_bQuiet)
		{
			...
			for (size_t i = 0; i < m_options.m_indices.size(); i++)
			{
				if (m_options.m_indices.test(i))
				{
					if (!indices.empty())
						indices += ", ";
					indices += MODIS::GetIndiceName(i);
				}
			}

			cout << "Output:  " << m_options.m_filesPath[CMODISIndicesOption::OUTPUT_FILE_PATH] << endl;
			cout << "From:    " << m_options.m_filesPath[CMODISIndicesOption::INPUT_FILE_PATH] << endl;
...
#pragma omp parallel for schedule(static, 1) num_threads( m_options.m_BLOCK_THREADS ) if (m_options.m_bMulti) 
	...
	ERMsg CMODISIndices::OpenAll(CMODISDataset& inputDS, CGDALDatasetEx& maskDS, CGDALDatasetEx& outputDS)
	{
		ERMsg msg;
		...
		msg = inputDS.OpenInputImage(m_options.m_filesPath[CMODISIndicesOption::INPUT_FILE_PATH], m_options);
		if (msg)
			...
			if (m_options.m_period.IsInit())
			{
				...
				for (size_t s = 0; s < scenesPeriod.size(); s++)
				{
					//if (s > 1 && (scenesPeriod[s].End() < scenesPeriod[s - 1].Begin()))
						//msg.ajoute("Scenes of the input MODIS images (" + GetFileName(m_options.m_filesPath[CMODISIndicesOption::INPUT_FILE_PATH]) + ") are not chronologically ordered.");
...
//				m_options.m_scenes[1] = inputDS.GetNbScenes() - 1;
		...
		if (msg && m_options.m_bCreateImage)
		{
			...
			size_t nbScenedProcess = m_options.m_scenes[1] - m_options.m_scenes[0] + 1;
			string filePath = m_options.m_filesPath[CMODISIndicesOption::OUTPUT_FILE_PATH];
			CMODISIndicesOption options = m_options;
			...
			for (size_t zz = 0; zz < nbScenedProcess; zz++)
			{
				...
				for (size_t i = 0; i < m_options.m_indices.size(); i++)
				{
					if (m_options.m_indices.test(i))
					{
						options.m_VRTBandsName += GetFileTitle(filePath) + "_" + uniqueSubName + "_" + MODIS::GetIndiceName(i) + ".tif|";
					}
				}
			}
	...
	void CMODISIndices::ReadBlock(int xBlock, int yBlock, CBandsHolder& bandHolder)
	{
#pragma omp critical(BlockIO)
	...
	void CMODISIndices::ProcessBlock(int xBlock, int yBlock, CBandsHolder& bandHolder, OutputData& outputData)
	{
...
#pragma omp atomic
		...
		CMODISWindow window = static_cast<CMODISWindow&>(bandHolder.GetWindow());
...
#pragma omp parallel for num_threads( m_options.BLOCK_CPU() ) if (m_options.m_bMulti) 
		for (int y = 0; y < blockSize.m_y; y++)
		{
			for (int x = 0; x < blockSize.m_x; x++)
			{
				...
				{
					vector< pair<CMODISPixel, size_t>> data;
					...
					{
						size_t z = m_options.m_scenes[0] + zz;
						//CMODISPixel pixel = ((CMODISWindow&)window).GetPixel(z, x, y);
						CMODISPixel pixel = ((CMODISWindow&)window).GetPixelMean(z, x, y, (int)m_options.m_weight.size(), m_options.m_weight);
				...
				{
					size_t z = m_options.m_scenes[0] + zz;
					CMODISPixel pixel = m_options.m_rings == 0 ? window.GetPixel(z, x, y) : window.GetPixelMean(z, x, y, (int)m_options.m_rings, m_options.m_weight);
					if (pixel.IsValid() && !bSpiking[zz])
					{
						...
						for (size_t i = 0; i < m_options.m_indices.size(); i++)
						{
							if (m_options.m_indices.test(i))
							{
								...
								outputData[zz][ii][xy] = pixel[MODIS::TIndices(i)];
								ii++;
							}
						}
					}
...
#pragma omp atomic 
	...
	void CMODISIndices::WriteBlock(int xBlock, int yBlock, CBandsHolder& bandHolder, CGDALDatasetEx& outputDS, OutputData& outputData)
	{
#pragma omp critical(BlockIO)
		{
	...
	void CMODISIndices::CloseAll(CGDALDatasetEx& inputDS, CGDALDatasetEx& maskDS, CGDALDatasetEx& outputDS)
	{
		inputDS.Close();
		maskDS.Close();
	...
	ERMsg CMODISIndices::CreateVirtual(CMODISDataset& inputDS)
	{
		ERMsg msg;
		...
		for (size_t zz = 0; zz < nbScenedProcess; zz++)
		{
			...
			for (size_t i = 0; i < m_options.m_indices.size(); i++)
			{
				if (m_options.m_indices.test(i))
				{
					...
					std::string filePath = m_options.m_filesPath[CMODISIndicesOption::OUTPUT_FILE_PATH];
					filePath = GetPath(filePath) + GetFileTitle(filePath) + "_" + subName + "_" + MODIS::GetIndiceName(i) + ".vrt";
					msg += inputDS.CreateIndices(z, filePath, (MODIS::TIndices)i);

					//Input : le vrt de nos 30 ans MODIS + le dd5, DEM, slope.Le Seuil du despike(ex. 0.75) et le seuil de tolérance(ex. 0.1)
