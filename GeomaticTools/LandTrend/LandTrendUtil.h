...
namespace LTR
{
	...
	//class CBoolArray : public CBoolArrayBase
	//{
	//public:
	//	//	using std::valarray<bool>();
	//	using CBoolArrayBase::CBoolArrayBase;
	//	using CBoolArrayBase::operator=;
	//
	//	size_t sum() const
	//	{
	//		size_t sum = 0;
	...
	{
		_ASSERTE(x_endpoints.size() == 2);
		_ASSERTE(y_endpoints.size() == 2);
	...
	public:
		...
		size_t n_obs() { return yfit.size(); }
		size_t n_segments() { return vertices.size() - 1; }
		size_t n_vertices() { return vertices.size(); }
	...
	CRealArray desawtooth(CRealArray vals, REAL_TYPE stopat = 0.9);
