...
int main(int argc, char* argv[])
{
	...
	std::valarray<bool> goods(true, x.size());
	//goods[0] = false;
	//goods[1] = false;
	//goods[2] = false;
	//goods[3] = false;
	...
	CBestModelInfo result = fit_trajectory_v2(x, y, goods,
		minneeded, background, modifier, seed,
	...
	//ofStream file;
	//file.open("E:/Landsat/Landsat(LandTrend)/test_landTrend.csv");
	//file << "x,y,obs_yfit,my_yfit" << endl;

	//for (size_t i = 0; i < reult.yfit.size(); i++)
	//{
	//	file << ToString(x[i], 3) << "," << ToString(y[i], 3) << "," << ToString(yfit[i], 3) << "," << ToString(reult.yfit[i], 3) << endl;
	//}
